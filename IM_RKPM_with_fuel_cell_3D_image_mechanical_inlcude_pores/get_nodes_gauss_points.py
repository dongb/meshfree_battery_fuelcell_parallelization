...
@jit
def get_x_nodes_fuel_cell_2d_toy(x_min_electrolyte,x_max_electrolyte,y_min_electrolyte,y_max_electrolyte,x_min_electrode,x_max_electrode,y_min_electrode,y_max_electrode, n_intervals):
    ...
    x_nodes_mechanical = []  # includes all nodes in electrolyte and electrode as well as nodes at interface. Interface nodes will be domain nodes for mechanicsl simulation
    ...
    for i in range(2*n_nodes-1):
        for j in range(n_nodes):
            x_nodes_mechanical.append([x_min+(x_max-x_min)/n_intervals/2*i, y_min+(y_max-y_min)/n_intervals*j])
    
    
    # in electrolyte or electrode
    for j in range(n_nodes):
        for i in range(n_nodes):
            x_nodes_electrolyte.append([x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j])
            x_nodes_electrode.append([x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*i, y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*j])

            if j < n_intervals and i < n_intervals:
                cell_nodes_electrolyte_x.append([x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i, x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*(i+1), x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*(i+1), x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i])
                cell_nodes_electrolyte_y.append([y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*(j+1), y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*(j+1)])
                cell_nodes_electrode_x.append([x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*i, x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*(i+1), x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*(i+1), x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*i])
                cell_nodes_electrode_y.append([y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*j, y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*j, y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*(j+1), y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*(j+1)])
           
            if i == 0 and j < n_intervals:
                cell_nodes_left_electrolyte_y.append([y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*(j+1)])
                nodes_id_left_electrolyte.append(nodes_id_electrolyte)

            if i == n_nodes-1 and j < n_intervals:
                cell_nodes_right_electrode_y.append([y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*j, y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*(j+1)])
                nodes_id_right_electrode.append(nodes_id_electrode)
            if (j == 0 or j == n_nodes-1) and (i == n_nodes-1):
                    point_source_coords.append([x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j])
               
            nodes_id_electrolyte += 1
            nodes_id_electrode += 1

    return x_nodes_mechanical, x_nodes_electrolyte, x_nodes_electrode, point_source_coords, point_fixed_coords, nodes_id_left_electrolyte, nodes_id_right_electrode, cell_nodes_electrolyte_x, cell_nodes_electrolyte_y, cell_nodes_electrode_x, cell_nodes_electrode_y, cell_nodes_left_electrolyte_y, cell_nodes_right_electrode_y


# @jit
def get_x_nodes_fuel_cell_3d_toy(x_min_electrolyte,x_max_electrolyte,y_min_electrolyte,y_max_electrolyte,z_min_electrolyte, z_max_electrolyte, x_min_electrode,x_max_electrode,y_min_electrode,y_max_electrode,z_min_electrode, z_max_electrode, n_intervals):
    ...
    for i in range(n_nodes):
        for j in range(2*n_nodes-1):
            for k in range(n_nodes):
                x_nodes_mechanical.append([x_min+(x_max-x_min)/n_intervals*i, y_min+(y_max-y_min)/2/n_intervals*j, z_min+(z_max-z_min)/n_intervals*k])
                
                if j == 0 and k == 0 and i < n_intervals:
                    segments_fixed_coords.append([x_min+(x_max-x_min)/n_intervals*i, 0, 0, x_min+(x_max-x_min)/n_intervals*(i+1), 0 ,0])
    
    # in electrolyte
    for j in range(n_nodes):
        for i in range(n_nodes):
            for k in range(n_nodes):
                x_nodes_electrolyte.append([x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j, z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*k])
                x_nodes_electrode.append([x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*i, y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*j, z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*k])
                
                if i < n_intervals and j < n_intervals and k < n_intervals:
                    cell_nodes_electrolyte_x.append([x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i, x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*(i+1), x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*(i+1), x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i, \
                                                     x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i, x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*(i+1), x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*(i+1), x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i])
                    
                    cell_nodes_electrolyte_y.append([y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*(j+1), y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*(j+1), \
                                                     y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*(j+1), y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*(j+1)])

                    cell_nodes_electrolyte_z.append([z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*k, z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*k, z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*k, z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*k, \
                                                     z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*(k+1), z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*(k+1), z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*(k+1), z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*(k+1)])

                    cell_nodes_electrode_x.append([x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*i, x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*(i+1), x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*(i+1), x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*i, \
                                                     x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*i, x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*(i+1), x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*(i+1), x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*i])
                    
                    cell_nodes_electrode_y.append([y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*j, y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*j, y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*(j+1), y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*(j+1), \
                                                     y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*j, y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*j, y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*(j+1), y_min_electrode+(y_max_electrode-y_min_electrode)/n_intervals*(j+1)])

                    cell_nodes_electrode_z.append([z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*k, z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*k, z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*k, z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*k, \
                                                     z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*(k+1), z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*(k+1), z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*(k+1), z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*(k+1)])

                if j == 0:
                    nodes_id_left_electrolyte.append(nodes_id_electrolyte)
                    if i < n_intervals and k < n_intervals:
                        cell_nodes_left_electrolyte_x.append([x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i, x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*(i+1), x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*(i+1), x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i])
                        cell_nodes_left_electrolyte_z.append([z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*k, z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*k, z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*(k+1), z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*(k+1)])
                
                if j == n_nodes-1:
                    nodes_id_right_electrode.append(nodes_id_electrode)
                    if i < n_intervals and k < n_intervals:
                        cell_nodes_right_electrode_x.append([x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*i, x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*(i+1), x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*(i+1), x_min_electrode+(x_max_electrode-x_min_electrode)/n_intervals*i])
                        cell_nodes_right_electrode_z.append([z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*k, z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*k, z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*(k+1), z_min_electrode+(z_max_electrode-z_min_electrode)/n_intervals*(k+1)])
                    
                    if (i == 0 or i == n_nodes-1) and k < n_intervals:
                        segments_source_coords.append([x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j, z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*k, x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j, z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*(k+1)])
                    if (k == 0 or k == n_nodes-1) and i < n_intervals:
                        segments_source_coords.append([x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*i, y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j, z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*k, x_min_electrolyte+(x_max_electrolyte-x_min_electrolyte)/n_intervals*(i+1), y_min_electrolyte+(y_max_electrolyte-y_min_electrolyte)/n_intervals*j, z_min_electrolyte+(z_max_electrolyte-z_min_electrolyte)/n_intervals*k])
    ...
    for i in range(num_pixels_x):
        for j in range(num_pixels_y):
            for k in range(num_pixels_z):
                if j == 0 and k == 0:
                    segments_fixed.append([x_min+(x_max-x_min)/(num_pixels_x)*i, 0, 0, x_min+(x_max-x_min)/(num_pixels_x)*(i+1), 0, 0])
                ...
                if img_[i, j, k] == 2:
                    ...
                    if (j+1)<num_pixels_y and img_[tuple(surface_adjacent_pixel_index)] == 1: # electrolyte/electrode interface
                        cell_nodes_interface_electrode_electrolyte_x.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), x_min+(x_max-x_min)/(num_pixels_x)*(i), x_min+(x_max-x_min)/(num_pixels_x)*(i), x_min+(x_max-x_min)/(num_pixels_x)*(i+1)])
                ...
                if img_[i, j, k] == 1:
                    ...
                    if (j+1)<num_pixels_x and img_[tuple(surface_adjacent_pixel_index)] == 0: # electrolyte/electrode interface
                        cell_nodes_interface_electrode_pore_x.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), x_min+(x_max-x_min)/(num_pixels_x)*(i), x_min+(x_max-x_min)/(num_pixels_x)*(i), x_min+(x_max-x_min)/(num_pixels_x)*(i+1)])
...
def get_x_nodes_fuel_cell_2d_toy_image(x_min,x_max,y_min,y_max,num_pixels_xyz, img_):
    # x_min... are the range of the whole domain.
    ...
    # segments on triple junctions or with flux, line integral of point source. 
    point_source = [] # n by 6 array, n is the number of segments with flux, 2 points on this segments, 6 coordinates for 3d
    ...
    for i in range(num_pixels_x):
        for j in range(num_pixels_y):
            ...
            if img_[i, j] != 0:
                if [x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*j] not in x_nodes_mechanical:
                    x_nodes_mechanical.append([x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*j])
                    
                if [x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j)] not in x_nodes_mechanical:
                    x_nodes_mechanical.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j)])

                if [x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j+1)] not in x_nodes_mechanical:
                    x_nodes_mechanical.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                    
                if [x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)] not in x_nodes_mechanical:
                    x_nodes_mechanical.append([x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
            ...
            if img_[i, j] == 2:
                ...
                if i == 0:
                    cell_nodes_left_electrolyte_y.append([y_min+(y_max-y_min)/(num_pixels_y)*j, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                
                # check if each edge is a triple junction, point 1
                adjacent_pixel_index = np.array([[i,j-1],[i-1,j],[i-1,j-1]]) # 3 adjacent pixels
                ...
                if (0 in unique_id_djacent_pixels and 1 in unique_id_djacent_pixels): # or (1 in unique_id_djacent_pixels and check_if_edge): 
                    if [x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*j] not in point_source:
                        point_source.append([x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*j])
                
                # point 2
                adjacent_pixel_index = np.array([[i+1,j],[i+1,j-1],[i,j-1]]) # 3 adjacent pixels
                filter_mask = (
                        np.all(adjacent_pixel_index >= 0, axis=1) &  # all values non-negative
                        (adjacent_pixel_index[:, 0] <= num_pixels_x-1) &         # first column ≤ num_pixels_x
                        (adjacent_pixel_index[:, 1] <= num_pixels_y-1)         # second column ≤ num_pixels_y
                ...
                if (0 in unique_id_djacent_pixels and 1 in unique_id_djacent_pixels):# or (1 in unique_id_djacent_pixels and check_if_edge): 
                    if [x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*j] not in point_source:
                        point_source.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*j])
                
                # point 3
                adjacent_pixel_index = np.array([[i,j+1],[i+1,j+1],[i+1,j]]) # 3 adjacent pixels
                filter_mask = (
                        np.all(adjacent_pixel_index >= 0, axis=1) &  # all values non-negative
                        (adjacent_pixel_index[:, 0] <= num_pixels_x-1) &         # first column ≤ num_pixels_x
                        (adjacent_pixel_index[:, 1] <= num_pixels_y-1)         # second column ≤ num_pixels_y
                ...
                if (0 in unique_id_djacent_pixels and 1 in unique_id_djacent_pixels):# or (1 in unique_id_djacent_pixels and check_if_edge): 
                    if [x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j+1)] not in point_source:
                        point_source.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                ...
                if (0 in unique_id_djacent_pixels and 1 in unique_id_djacent_pixels):# or (1 in unique_id_djacent_pixels and check_if_edge): 
                    if [x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)] not in point_source:
                        point_source.append([x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                
            
                
                if [x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*j] not in x_nodes_electrolyte:
                    x_nodes_electrolyte.append([x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*j])
                    # if on left boundary
                    if j == 0:
                        nodes_id_left_electrolyte.append(nodes_id_electrolyte)
                ...
                if [x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j)] not in x_nodes_electrolyte:
                    x_nodes_electrolyte.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j)])
                # if on left boundary
                    if j == 0:
                        nodes_id_left_electrolyte.append(nodes_id_electrolyte)
                ...
                if [x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j+1)] not in x_nodes_electrolyte:
                    x_nodes_electrolyte.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                    
                    nodes_id_electrolyte += 1

                if [x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)] not in x_nodes_electrolyte:
                    x_nodes_electrolyte.append([x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                ...
                if (i-1)>=0 and img_[tuple(surface_adjacent_pixel_index)] == 1: # electrolyte/electrode interface
                    cell_nodes_interface_electrode_electrolyte_x.append([x_min+(x_max-x_min)/(num_pixels_x)*i, x_min+(x_max-x_min)/(num_pixels_x)*i])
                    cell_nodes_interface_electrode_electrolyte_y.append([y_min+(y_max-y_min)/(num_pixels_y)*j, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                   
                # surface 2
                surface_adjacent_pixel_index = np.array([i+1,j]) # 3 adjacent pixels
                if (i+1)<num_pixels_x and img_[tuple(surface_adjacent_pixel_index)] == 1: # electrolyte/electrode interface
                    cell_nodes_interface_electrode_electrolyte_x.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), x_min+(x_max-x_min)/(num_pixels_x)*(i+1)])
                    cell_nodes_interface_electrode_electrolyte_y.append([y_min+(y_max-y_min)/(num_pixels_y)*j, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                ...
                if (j-1)>=0 and img_[tuple(surface_adjacent_pixel_index)] == 1: # electrolyte/electrode interface
                    cell_nodes_interface_electrode_electrolyte_x.append([x_min+(x_max-x_min)/(num_pixels_x)*i, x_min+(x_max-x_min)/(num_pixels_x)*(i+1)])
                    cell_nodes_interface_electrode_electrolyte_y.append([y_min+(y_max-y_min)/(num_pixels_y)*j, y_min+(y_max-y_min)/(num_pixels_y)*(j)])
                   
                # surface 4
                surface_adjacent_pixel_index = np.array([i,j+1]) # 3 adjacent pixels
                if (j+1)<num_pixels_y and img_[tuple(surface_adjacent_pixel_index)] == 1: # electrolyte/electrode interface
                    cell_nodes_interface_electrode_electrolyte_x.append([x_min+(x_max-x_min)/(num_pixels_x)*(i), x_min+(x_max-x_min)/(num_pixels_x)*(i+1)])
                    cell_nodes_interface_electrode_electrolyte_y.append([y_min+(y_max-y_min)/(num_pixels_y)*(j+1), y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
            ...
            if img_[i, j] == 1:
                ...
                if i+1 == num_pixels_x:
                    cell_nodes_right_electrode_y.append([y_min+(y_max-y_min)/(num_pixels_y)*j, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                
                if [x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*j] not in x_nodes_electrode:
                    x_nodes_electrode.append([x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*j])
                    nodes_id_electrode += 1

                if [x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j)] not in x_nodes_electrode:
                    x_nodes_electrode.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j)])
                    nodes_id_electrode += 1

                if [x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j+1)] not in x_nodes_electrode:
                    x_nodes_electrode.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                # if on right boundary
                    if j+1 == num_pixels_y:
                        nodes_id_right_electrode.append(nodes_id_electrode)
                    nodes_id_electrode += 1

                if [x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)] not in x_nodes_electrode:
                    x_nodes_electrode.append([x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                # if on right boundary
                    if j+1 == num_pixels_y:
                        nodes_id_right_electrode.append(nodes_id_electrode)
                    nodes_id_electrode += 1
                ...
                surface_adjacent_pixel_index = np.array([i-1,j]) # 3 adjacent pixels
                if (i-1)>=0 and img_[tuple(surface_adjacent_pixel_index)] == 0: # electrolyte/electrode interface
                    cell_nodes_interface_electrode_pore_x.append([x_min+(x_max-x_min)/(num_pixels_x)*i, x_min+(x_max-x_min)/(num_pixels_x)*i])
                    cell_nodes_interface_electrode_pore_y.append([y_min+(y_max-y_min)/(num_pixels_y)*j, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                  
                
                # surface 2
                surface_adjacent_pixel_index = np.array([i+1,j]) # 3 adjacent pixels
                if (i+1)<num_pixels_x and img_[tuple(surface_adjacent_pixel_index)] == 0: # electrolyte/electrode interface
                    cell_nodes_interface_electrode_pore_x.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), x_min+(x_max-x_min)/(num_pixels_x)*(i+1)])
                    cell_nodes_interface_electrode_pore_y.append([y_min+(y_max-y_min)/(num_pixels_y)*j, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                ...
                surface_adjacent_pixel_index = np.array([i,j-1]) # 3 adjacent pixels
                if (j-1)>=0 and img_[tuple(surface_adjacent_pixel_index)] == 0: # electrolyte/electrode interface
                    cell_nodes_interface_electrode_pore_x.append([x_min+(x_max-x_min)/(num_pixels_x)*i, x_min+(x_max-x_min)/(num_pixels_x)*(i+1)])
                    cell_nodes_interface_electrode_pore_y.append([y_min+(y_max-y_min)/(num_pixels_y)*j, y_min+(y_max-y_min)/(num_pixels_y)*(j)])
                ...
                if (j+1)<num_pixels_x and img_[tuple(surface_adjacent_pixel_index)] == 0: # electrolyte/electrode interface
                    cell_nodes_interface_electrode_pore_x.append([x_min+(x_max-x_min)/(num_pixels_x)*(i), x_min+(x_max-x_min)/(num_pixels_x)*(i+1)])
                    cell_nodes_interface_electrode_pore_y.append([y_min+(y_max-y_min)/(num_pixels_y)*(j+1), y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
            ...
            if img_[i, j] == 0:
                ...
                if i+1 == num_pixels_x:
                    cell_nodes_right_pore_y.append([y_min+(y_max-y_min)/(num_pixels_y)*j, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                
                if [x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*j] not in x_nodes_pore:
                    x_nodes_pore.append([x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*j])
                    nodes_id_pore += 1

                if [x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j)] not in x_nodes_pore:
                    x_nodes_pore.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j)])
                    nodes_id_pore += 1

                if [x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j+1)] not in x_nodes_pore:
                    x_nodes_pore.append([x_min+(x_max-x_min)/(num_pixels_x)*(i+1), y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                # if on right boundary
                    if j+1 == num_pixels_y:
                        nodes_id_right_pore.append(nodes_id_pore)
                    nodes_id_pore += 1

                if [x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)] not in x_nodes_pore:
                    x_nodes_pore.append([x_min+(x_max-x_min)/(num_pixels_x)*i, y_min+(y_max-y_min)/(num_pixels_y)*(j+1)])
                # if on right boundary
                    if j+1 == num_pixels_y:
                        nodes_id_right_pore.append(nodes_id_pore)
                    nodes_id_pore += 1

    return x_nodes_mechanical, x_nodes_electrolyte, x_nodes_electrode, x_nodes_pore, point_source, point_fixed, nodes_id_left_electrolyte, nodes_id_right_electrode, nodes_id_right_pore, cell_nodes_electrolyte_x, cell_nodes_electrolyte_y, cell_nodes_electrode_x, cell_nodes_electrode_y, cell_nodes_pore_x,cell_nodes_pore_y, cell_nodes_left_electrolyte_y, cell_nodes_right_electrode_y, cell_nodes_right_pore_y,\
...
def get_x_nodes_multi_grain(x_min, x_max, y_min, y_max, num_pixels_xyz, img_):
    ...
    return cell_nodes_list, grain_id, grain_id_bottom, grain_id_top, grain_id_left, grain_id_right, cell_shape, num_rec_cell, num_tri_cell, x_nodes, nodes_grain_id, bottom_boundary_cell_nodes_list, right_boundary_cell_nodes_list, top_boundary_cell_nodes_list, left_boundary_cell_nodes_list, repeated_vertex, interface_segments


# get all gauss points in domain, 3d domain
def x_G_and_def_J_time_weight_structured_3d(n_intervals, x_min,x_max,y_min,y_max,z_min,z_max,x_G_domain,weight_G_domain):
    ...
    for n in range(n_intervals):
        for m in range(n_intervals):
            for nm in range(n_intervals):
                ...
                z_ver_mn = np.array([z_min+nm*(z_max-z_min)/n_intervals, z_min+nm*(z_max-z_min)/n_intervals, z_min+nm*(z_max-z_min)/n_intervals, z_min+nm*(z_max-z_min)/n_intervals, z_min+(nm+1)*(z_max-z_min)/n_intervals,z_min+(nm+1)*(z_max-z_min)/n_intervals,z_min+(nm+1)*(z_max-z_min)/n_intervals,z_min+(nm+1)*(z_max-z_min)/n_intervals])
...
def x_G_b_and_det_J_b_time_weight_structured_3d(n_intervals,x_min,x_max, y_min,y_max,z_min,z_max, face_id,x_G_domain,weight_G_domain):
    x_G = []      # xy coordinates of gauss points in domain   
    det_J_time_weight = []    # determin of jacobian
    ...
    if face_id == 0 or face_id == 2:
        ...
        for n in range(n_intervals):
            for m in range(n_intervals):
                ...
                z_ver_mn = np.array([z_min+n*(z_max-z_min)/n_intervals, z_min+n*(z_max-z_min)/n_intervals, z_min+(n+1)*(z_max-z_min)/n_intervals, z_min+(n+1)*(z_max-z_min)/n_intervals],dtype=np.float64)
                # calculate the cy coordinates of gauss points in current integration domain
                for k in range(len(x_G_domain)):
    ...
    if face_id == 1 or face_id == 3:
        ...
        for n in range(n_intervals):
            for m in range(n_intervals):
                # in the mn (n^th row, m^th column) gauss integration domain, calculate the xy coordinates of each domain vertex
                y_ver_mn = np.array([y_min+m*(y_max-y_min)/n_intervals, y_min+(m+1)*(y_max-y_min)/n_intervals, y_min+(m+1)*(y_max-y_min)/n_intervals, y_min+m*(y_max-y_min)/n_intervals],dtype=np.float64)
                z_ver_mn = np.array([z_min+n*(z_max-z_min)/n_intervals, z_min+n*(z_max-z_min)/n_intervals, z_min+(n+1)*(z_max-z_min)/n_intervals, z_min+(n+1)*(z_max-z_min)/n_intervals],dtype=np.float64)
                # calculate the cy coordinates of gauss points in current integration domain
                for k in range(len(x_G_domain)):
...
@jit
def x_G_and_det_J_structured_line_3d(n_boundaries, n_intervals, x_interface, y_max, y_min, z_max,z_min, x_G_line, weight_G_line):
    ...
    for i in range(n_boundaries):
        for j in range(n_intervals):
            ...
            if i==1:              # right boundary
                z_ver_b = np.array([z_min+(z_max-z_min)/n_intervals*j, z_min+(z_max-z_min)/n_intervals*(j+1)])
            ...
            if i==2:              # top boundary
                ...
                for k in range(len(x_G_line)):
                    y_G_ij_k = (y_ver_b[1]-y_ver_b[0])/2*x_G_line[k]+(y_ver_b[1]+y_ver_b[0])/2  # if 
                    x_G_ij_k =x_interface
                    z_G_ij_k = z_max
                    x_G_b_line.append([x_G_ij_k, y_G_ij_k, z_G_ij_k])
...
# get all gauss points in domain, 2d domain
def x_G_and_def_J_time_weight_2d_fuelcell_domain(cell_nodes_x,cell_nodes_y,x_G_domain,weight_G_domain):
    x_G = []      # xy coordinates of gauss points in domain   
    det_J_time_weight = []    # determin of jacobian
...
@jit
def x_G_and_def_J_time_weight_2d_fuelcell_boundary(cell_nodes, x_coord, x_G_line, weight_G_line):
...
def x_G_b_and_det_J_b_time_weight_2d_fuelcell_boundary(cell_nodes_boundary_x,cell_nodes_boundary_y, x_G_line, weight_G_line):
    x_G_b_line = []         
    det_J_b_time_weight_line = []    # determin of jacobian
