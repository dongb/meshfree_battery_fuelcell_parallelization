...
def allign_G(x_G,x_G_kristen, kristen_data):
    ...
    for i in range(np.shape(x_G)[0]):
        for j in range(np.shape(x_G)[0]):
            ...
            if abs(x_G[i, 0] - x_G_kristen[j, 0]) < 1.0e-13 and abs(x_G[i, 1] - x_G_kristen[j, 1]) < 1.0e-13:
...
alligned_x_G, alligned_heavy, alligned_heavy_px, alligned_heavy_py = allign_G(x_G, x_G_kristen, kristen_data)
