...
def func_1(a1, a3, a2, a4, a, M1, M2, M3, a6_num, a5, a6):
    ...
    for i in range(np.shape(a1)[0]):
        ...
        if min_index in positive_index: # if the smallest distance is between AD, D in between BC
            x_coor_min_point_segment = BA_dot_unit_BC_times_unit_BC[min_index, 0] + a6[min_index,0]
            y_coor_min_point_segment = BA_dot_unit_BC_times_unit_BC[min_index, 1] + a6[min_index,1]
        ...
        for j in range(np.shape(a2)[0]):
            ...
            z_ij = (((a1[i,0]-a2[j,0])**2+(a1[i,1]-a2[j,1])**2)**0.5)/a[j]
            z_ij_P_x = (a1[i,0]-a2[j,0])/(a[j]*z_ij*a[j]+2.220446049250313e-16)              # partial z partial x, add the small number to force the term with machine accuracy
            z_ij_P_y = (a1[i,1]-a2[j,1])/(a[j]*z_ij*a[j]+2.220446049250313e-16)              # partial z partial y

            x_I = a2[j]

            H_sacling_factor = 1.0e-6

            H_T = np.array([1, (a1[i][0]-x_I[0])/H_sacling_factor, (a1[i][1]-x_I[1])/H_sacling_factor],dtype=np.float64)
            H = np.transpose(H_T)
            ...
            if z_ij >= 0 and z_ij <= 1.0:
                ...
                for i_nodes in range(a6_num*2):
                    # print('yyy')
                    if abs(a2[j,0] - a5[i_nodes, 0])<1e-10 and abs(a2[j,1] - a5[i_nodes, 1])<1e-10:
                ...
                if node_not_on_interface == 'True':
                    if a4[j] == a3[i]:
                        ...
                        n4.append(phi_P_x_ij*heaviside+phi_ij*heaviside_P_x)    # partial phi partial x
                        n5.append(phi_P_y_ij*heaviside+phi_ij*heaviside_P_y)    # partial phi partial y
                        ...
                        for ii in range(3):
                            for jj in range(3):
                                M1[i][ii][jj] = M1[i][ii][jj] + H[ii]*H_T[jj]*phi_ij*heaviside
                                M2[i][ii][jj] = M2[i][ii][jj] + H[ii]*H_T[jj]*(phi_P_x_ij*heaviside+phi_ij*heaviside_P_x) + H_P_x[ii]*H_T[jj]*phi_ij*heaviside + H[ii]*HT_P_x[jj]*phi_ij*heaviside
                                M3[i][ii][jj] = M3[i][ii][jj] + H[ii]*H_T[jj]*(phi_P_y_ij*heaviside+phi_ij*heaviside_P_y) + H_P_y[ii]*H_T[jj]*phi_ij*heaviside + H[ii]*HT_P_y[jj]*phi_ij*heaviside
                    ...
                    for ii in range(3):
                        for jj in range(3):
                            ...
                            M2[i][ii][jj] = M2[i][ii][jj] + H[ii]*H_T[jj]*phi_P_x_ij + H_P_x[ii]*H_T[jj]*phi_ij + H[ii]*HT_P_x[jj]*phi_ij
                            M3[i][ii][jj] = M3[i][ii][jj] + H[ii]*H_T[jj]*phi_P_y_ij + H_P_y[ii]*H_T[jj]*phi_ij + H[ii]*HT_P_y[jj]*phi_ij
...
def func_2(a1,a2, n1_num, H0, M1, M2, M3, n3, n4, n5, n1, n2):
    ...
    for ii in range(n1_num):
        ...
        s1_ij = np.dot((np.dot((H0).astype(np.float64), (np.linalg.inv(M1[i])).astype(np.float64))).astype(np.float64), H.astype(np.float64))*n3[ii]
        
       
        M_inv_2 = -np.dot(np.dot(np.linalg.inv(M1[i].astype(np.float64)).astype(np.float64), M2[i].astype(np.float64)), np.linalg.inv(M1[i].astype(np.float64)).astype(np.float64))
        M_inv_3 = -np.dot(np.dot(np.linalg.inv(M1[i].astype(np.float64)).astype(np.float64), M3[i].astype(np.float64)), np.linalg.inv(M1[i].astype(np.float64)).astype(np.float64))
        s2_ij = np.dot((np.dot((H0).astype(np.float64), (np.linalg.inv(M1[i])).astype(np.float64))).astype(np.float64), H.astype(np.float64))*n4[ii] +\
                                np.dot((np.dot((H0).astype(np.float64), (M_inv_2).astype(np.float64))).astype(np.float64), H.astype(np.float64))*n3[ii] +\
                                np.dot((np.dot((H0).astype(np.float64), (np.linalg.inv(M1[i])).astype(np.float64))).astype(np.float64), H_P_x.astype(np.float64))*n3[ii]
        s3_ij = np.dot((np.dot((H0).astype(np.float64), (np.linalg.inv(M1[i])).astype(np.float64))).astype(np.float64), H.astype(np.float64))*n5[ii] +\
                                np.dot((np.dot((H0).astype(np.float64), (M_inv_3).astype(np.float64))).astype(np.float64), H.astype(np.float64))*n3[ii] +\
                                np.dot((np.dot((H0).astype(np.float64), (np.linalg.inv(M1[i])).astype(np.float64))).astype(np.float64), H_P_y.astype(np.float64))*n3[ii]
