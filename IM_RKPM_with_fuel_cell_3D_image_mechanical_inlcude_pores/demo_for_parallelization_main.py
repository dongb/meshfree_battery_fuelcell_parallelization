...
for i in range(a2_num):
    ...
    index_four_smallest = sorted(range(len(di)), key=lambda sub: di[sub])[:5]  # get the index of the four smallest index, the first one is always zero, so 5 here

    h[i] = di[index_four_smallest][di[index_four_smallest].tolist().index(max(di[index_four_smallest]))]
