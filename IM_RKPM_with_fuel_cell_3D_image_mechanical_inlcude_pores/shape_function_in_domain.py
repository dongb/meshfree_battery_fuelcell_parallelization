        ...
        for i in range(np.shape(x_G)[0]):
            for j in range(np.shape(x_nodes)[0]):
                ...
                if np.shape(M)[1] == 3:
                    ...
                    HT_P_y = np.array([0,0,1],dtype=np.float64)/H_sacling_factor # partial H partial y
                if np.shape(M)[1] == 4:
                    z_ij = (((x_G[i,0]-x_nodes[j,0])**2+(x_G[i,1]-x_nodes[j,1])**2+(x_G[i,2]-x_nodes[j,2])**2)**0.5)/a[j]
                    z_ij_P_x = (x_G[i,0]-x_nodes[j,0])/(a[j]*z_ij*a[j]+2.220446049250313e-16)              # partial z partial x, add the small number to force the term with machine accuracy
                    z_ij_P_y = (x_G[i,1]-x_nodes[j,1])/(a[j]*z_ij*a[j]+2.220446049250313e-16)              # partial z partial y
                    z_ij_P_z = (x_G[i,2]-x_nodes[j,2])/(a[j]*z_ij*a[j]+2.220446049250313e-16)              # partial z partial y
                    ...
                    HT_P_x = np.array([0,1,0,0],dtype=np.float64)/H_sacling_factor # partial H partial x
                    HT_P_y = np.array([0,0,1,0],dtype=np.float64)/H_sacling_factor # partial H partial y
                    HT_P_z = np.array([0,0,0,1],dtype=np.float64)/H_sacling_factor # partial H partial z
                ...
                if z_ij >= 0 and z_ij <= 1.0:
                    ...
                    for ii in range(np.shape(M)[1]):
                        for jj in range(np.shape(M)[1]):
                            ...
                            if np.shape(M)[1] == 4:
                                M_P_z[i][ii][jj] = M_P_z[i][ii][jj] + H[ii]*H_T[jj]*phi_P_z_ij + H_P_z[ii]*H_T[jj]*phi_ij + H[ii]*HT_P_z[jj]*phi_ij
...
def shape_grad_shape_func(x_G,x_nodes, num_non_zero_phi_a,HT0, M, M_P_x, M_P_y, differential_method, HT1, HT2, phi_nonzerovalue_data,phi_P_x_nonzerovalue_data,phi_P_y_nonzerovalue_data, phi_nonzero_index_row, phi_nonzero_index_column, det_J_time_weight, IM_RKPM, M_P_z=None, HT3 = None,phi_P_z_nonzerovalue_data=None):
    ...
    for ii in range(num_non_zero_phi_a):
        ...
        if np.shape(M)[1] == 4:
            ...
            HT_P_x = np.array([0,1,0,0],dtype=np.float64)/H_sacling_factor # partial H partial x
            HT_P_y = np.array([0,0,1,0],dtype=np.float64)/H_sacling_factor # partial H partial y
            HT_P_z = np.array([0,0,0,1],dtype=np.float64)/H_sacling_factor # partial H partial y
            H_P_z = np.transpose(HT_P_z)
        ...
        if differential_method =='implicite' and IM_RKPM == 'False':
            ...
            if np.shape(M) == 4:
                grad_shape_func_z_ij = np.dot((np.dot((HT3).astype(np.float64), (np.linalg.inv(M[i])).astype(np.float64))).astype(np.float64), H.astype(np.float64))*phi_nonzerovalue_data[ii]
            ...
            if differential_method =='direct' or IM_RKPM == 'True':
                ...
                if np.shape(M)[1] == 4:
                    M_inv_P_z_i = -np.dot(np.dot(np.linalg.inv(M[i].astype(np.float64)).astype(np.float64), M_P_z[i].astype(np.float64)), np.linalg.inv(M[i].astype(np.float64)).astype(np.float64))
                    grad_shape_func_z_ij = np.dot((np.dot((HT0).astype(np.float64), (np.linalg.inv(M[i])).astype(np.float64))).astype(np.float64), H.astype(np.float64))*phi_P_z_nonzerovalue_data[ii] +\
                                       np.dot((np.dot((HT0).astype(np.float64), (M_inv_P_z_i).astype(np.float64))).astype(np.float64), H.astype(np.float64))*phi_nonzerovalue_data[ii] +\
                                       np.dot((np.dot((HT0).astype(np.float64), (np.linalg.inv(M[i])).astype(np.float64))).astype(np.float64), H_P_z.astype(np.float64))*phi_nonzerovalue_data[ii]
        ...
        if np.shape(M)[1] == 4:
            ...
            grad_shape_func_z_times_det_J_time_weight_value.append(grad_shape_func_z_ij*det_J_time_weight[i])


    return shape_func_value, shape_func_times_det_J_time_weight_value, grad_shape_func_x_value, grad_shape_func_y_value,grad_shape_func_z_value, grad_shape_func_x_times_det_J_time_weight_value, grad_shape_func_y_times_det_J_time_weight_value, grad_shape_func_z_times_det_J_time_weight_value
