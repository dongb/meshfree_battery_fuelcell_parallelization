...
def compute_phi_M(x_G, Gauss_grain_id, x_nodes,nodes_grain_id, a, M, M_P_x, M_P_y, num_interface_segments, interface_nodes, BxByCxCy, IM_RKPM):
    ...
    for i in range(np.shape(x_G)[0]):
        for j in range(np.shape(x_nodes)[0]):
            ...
            H_T = np.array([1, (x_G[i][0]-x_I[0]), (x_G[i][1]-x_I[1])],dtype=np.float64) # scaling factor 1e-6
            H = np.transpose(H_T)
    ...
    return phi_nonzero_index_row, phi_nonzero_index_column, phi_nonzerovalue_data,phi_P_x_nonzerovalue_data, phi_P_y_nonzerovalue_data, M, M_P_x, M_P_y, heaviside_list, heaviside_plotting_points
...
def shape_grad_shape_func(x_G,x_nodes, num_non_zero_phi_a,HT0, M, M_P_x, M_P_y, differential_method, HT1, HT2, phi_nonzerovalue_data,phi_P_x_nonzerovalue_data,phi_P_y_nonzerovalue_data, phi_nonzero_index_row, phi_nonzero_index_column, det_J_time_weight, IM_RKPM):
    ...
    for ii in range(num_non_zero_phi_a):
        ...
        x_I = x_nodes[j]
        H_T = np.array([1, (x_G[i][0]-x_I[0]), (x_G[i][1]-x_I[1])],dtype=np.float64)
        H = np.transpose(H_T)
        HT_P_x = np.array([0,1,0],dtype=np.float64) # partial H partial x
        HT_P_y = np.array([0,0,1],dtype=np.float64) # partial H partial y
    ...
    return shape_func_value, grad_shape_func_x_value, grad_shape_func_y_value, 
