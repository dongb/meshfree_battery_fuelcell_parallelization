...
grad_shape_func_y = np.loadtxt('grad_shape_func_y_int.txt')
...
x_nodes_interface = np.loadtxt('x_nodes_interface.txt')
x_nodes = np.loadtxt('x_nodes.txt')

x_nodes_interface_kristen = np.loadtxt('x_nodes_interface_kristen.txt', delimiter=',')
x_nodes_kristen = np.loadtxt('x_nodes_kristen.txt', delimiter=',')

shape_func_kristen = np.loadtxt('shpIMRK_int.txt', delimiter=',')
grad_shape_func_x_kristen = np.loadtxt('dshpxIMRK_int.txt', delimiter=',')
grad_shape_func_y_kristen = np.loadtxt('dshpyIMRK_int.txt', delimiter=',')

print('G_shape', np.shape(x_nodes_interface), np.shape(x_nodes_interface_kristen))
...
num_nodes_interface = np.shape(x_nodes_interface)[0]
num_nodes = np.shape(x_nodes)[0]

# @jit

def allign_x_nodes_interface(num_nodes_interface, num_nodes, x_nodes_interface, x_nodes, x_nodes_interface_kristen, x_nodes_kristen, shape_func, shape_func_kristen, grad_shape_func_x, grad_shape_func_x_kristen, grad_shape_func_y, grad_shape_func_y_kristen):
    alligned_shape_func = np.zeros((num_nodes_interface, num_nodes))
    inter_alligned_shape_func = np.zeros((num_nodes_interface, num_nodes))
    alligned_grad_shape_func_x = np.zeros((num_nodes_interface, num_nodes))
    inter_alligned_grad_shape_func_x = np.zeros((num_nodes_interface, num_nodes))
    alligned_grad_shape_func_y = np.zeros((num_nodes_interface, num_nodes))
    inter_alligned_grad_shape_func_y = np.zeros((num_nodes_interface, num_nodes))
    ...
    alligned_x_nodes_interface_kristen = []
    ...
    for i in range(num_nodes_interface):
        for j in range(num_nodes_interface):
            if abs(x_nodes_interface[i, 0] - x_nodes_interface_kristen[j, 0]) < 1e-13 and abs(x_nodes_interface[i, 1] - x_nodes_interface_kristen[j, 1]) < 1e-13:
                alligned_x_nodes_interface_kristen.append([x_nodes_interface[i, 0], x_nodes_interface[i, 1]])
    ...
    for i in range(num_nodes_interface):
        for j in range(num_nodes):
            if abs(alligned_shape_func[i][j]-shape_func[i][j])>1.0e-5:
                ...
                mismatch_shape.append(abs(alligned_shape_func[i][j]-shape_func[i][j]))
            if abs(alligned_grad_shape_func_x[i][j]-grad_shape_func_x[i][j])> 1.0e-15:
                aa_grad_shape_x += 1
                mismatched_index_grad_shape_x.append([i,j])
                mismatch_grad_x.append(abs(alligned_grad_shape_func_x[i][j]-grad_shape_func_x[i][j]))
            if abs(alligned_grad_shape_func_y[i][j]-grad_shape_func_y[i][j])> 1.0e-15:
                aa_grad_shape_y += 1
    ...
    return alligned_x_nodes_kristen, alligned_x_nodes_interface_kristen, mismatch_grad_x, mismatch_grad_y, aa_shape, aa_grad_shape_x, aa_grad_shape_y, mismatched_index_shape, mismatched_index_grad_shape_x, mismatched_index_grad_shape_y, alligned_shape_func, alligned_grad_shape_func_x, alligned_grad_shape_func_y

# alligned_shape_func, alligned_grad_shape_func_x, alligned_grad_shape_func_y = 
alligned_x_nodes_kristen, alligned_x_nodes_interface_kristen,mismatch_grad_x, mismatch_grad_y, aa_shape, aa_grad_shape_x, aa_grad_shape_y, mismatched_index_shape, mismatched_index_grad_shape_x, mismatched_index_grad_shape_y, alligned_shape_func, alligned_grad_shape_func_x, alligned_grad_shape_func_y =  allign_x_nodes_interface(num_nodes_interface, num_nodes, x_nodes_interface, x_nodes, x_nodes_interface_kristen, x_nodes_kristen, shape_func, shape_func_kristen, grad_shape_func_x, grad_shape_func_x_kristen, grad_shape_func_y, grad_shape_func_y_kristen)
...
# print(alligned_shape_func[100,100])
...
print(np.shape(np.array(mismatched_index_grad_shape_x)))
print(np.shape(np.array(mismatched_index_grad_shape_y)))


plt.figure()
# # plt.scatter(x_G[:, 0], x_G[:, 1], c=np.linalg.norm(x_G-np.asarray(alligned_x_G_kristen),2,axis=1))
plt.scatter(x_nodes_interface[np.array(mismatched_index_grad_shape_x)[:,0], 0], x_nodes_interface[np.array(mismatched_index_grad_shape_x)[:,0], 1], c=mismatch_grad_x)
# plt.colorbar()
# plt.title('log(difference of x_G)')
# # plt.show()
# plt.figure()
...
plt.figure()
plt.scatter(x_nodes_interface[np.array(mismatched_index_grad_shape_y)[:,1], 0], x_nodes_interface[np.array(mismatched_index_grad_shape_y)[:,1], 1], c = mismatch_grad_y)
plt.colorbar()
